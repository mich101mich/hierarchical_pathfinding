...
impl Chunk {
    #[track_caller]
    #[allow(clippy::ref_option, reason = "Symmetry to get_at_mut")]
    ...
    pub(super) fn exit_at_mut(&mut self, (x, y): Point) -> &mut Option<Exit> {
        if y == 0 {
            ...
            self.bottom.get_mut(x)
        } else if x == 0 {
            self.left.get_mut(y - 1)
        } else if x == CHUNK_SIZE - 1 {
            self.right.get_mut(y - 1)
        } else {
            panic!("Invalid exit position");
        }
        .unwrap()
    ...
    pub(super) fn border_iterator(&self) -> impl Iterator<Item = (Point, &Option<Exit>)> {
        ...
        let top_row = (0..=right).map(move |x| ((x, 0), &self.top[x]));
        let bottom_row = (0..=right).map(move |x| ((x, bottom), &self.bottom[x]));
        let right_col = (1..bottom).map(move |y| ((right, y), &self.right[y - 1]));
        let left_col = (1..bottom).map(move |y| ((0, y), &self.left[y - 1]));
